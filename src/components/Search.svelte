<script lang="ts">
  ...
  async function search(q: string) {
    if (!q.trim()) { hits = []; statusMsg = ''; return; }
    loading = true;
    try {
  ...
  function onKeydown(e: KeyboardEvent) {
    if (!open) return;
    if (e.key === 'ArrowDown') { e.preventDefault(); selectedIndex = Math.min(selectedIndex + 1, hits.length - 1); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); selectedIndex = Math.max(selectedIndex - 1, -1); }
    else if (e.key === 'Enter' && selectedIndex >= 0 && hits[selectedIndex]) navigate(hits[selectedIndex]);
  ...
  function onGlobalKeydown(e: KeyboardEvent) {
    if ((e.metaKey || e.ctrlKey) && e.key === 'k') { e.preventDefault(); open ? hide() : show(); }
  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-3 px-4 py-3 border-b border-edge">
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" class="w-4 h-4 shrink-0 fg-weak" aria-hidden="true">
        ...
        {#if loading}
          ...
          <svg class="w-4 h-4 fg-weak animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            ...
            <li id="search-hit-{i}">
              ...
              >
                <div class="flex items-center gap-2">
                  {#if hit.category}
                    <span class="font-mono text-[10px] fg-accent uppercase tracking-[0.06em] font-semibold">{hit.category}</span>
                  {/if}
        ...
        {:else if query.trim() && !loading}
          <li>
            <div class="px-4 py-8 text-center">
              <p class="font-mono text-[13px] fg-weak">No results for <span class="fg-strong">"{query}"</span></p>
            </div>
          </li>
        {:else if !query.trim()}
          <li>
            <div class="px-4 py-6">
              <p class="font-mono text-[11px] fg-weak uppercase tracking-[0.08em] mb-3">Quick navigation</p>
              <div class="flex flex-wrap gap-2">
                {#each ['/articles', '/talks', '/videos', '/services', '/about'] as href}
                  <a {href} onclick={hide} class="font-mono text-[11px] fg-weak border border-edge rounded-[3px] px-2 py-1 no-underline transition-colors hover:border-accent hover:fg-accent">
                    {href}
                  </a>
                {/each}
              </div>
      ...
      <!-- Footer -->
      <div class="flex items-center gap-4 px-4 py-2 border-t border-edge-weak bg-surface-weak" aria-hidden="true">
