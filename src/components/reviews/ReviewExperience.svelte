<script lang="ts">
  ...
  onMount(() => {
    ...
    const updateActiveSection = () => {
      ...
      const activeIndex = Math.max(0, sectionElements.findIndex(el => el.id === activeSectionId));

      if (crossedIndex > activeIndex) {
        if (sectionElements[crossedIndex].getBoundingClientRect().top - topOffset <= -switchThreshold) {
          activeSectionId = sectionElements[crossedIndex].id;
        }
      } else if (crossedIndex < activeIndex) {
        if (sectionElements[activeIndex].getBoundingClientRect().top - topOffset >= switchThreshold) {
          activeSectionId = sectionElements[crossedIndex].id;
        }
      }
      rafId = 0;
...
<style>
  ...
  .section-nav li + li { margin-top: 4px; }
  ...
  @media (max-width: 720px) {
    ...
    .review-footer { padding: 32px 20px 48px; }
  }
