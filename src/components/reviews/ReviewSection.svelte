<script lang="ts">
  ...
  function getTabId(tab: TabKey) { return `${section.id}-${tab}-tab`; }
  function getPanelId(tab: TabKey) { return `${section.id}-${tab}-panel`; }
  function activateTab(tab: TabKey) { activeTab = tab; }

  function onTabKeydown(event: KeyboardEvent) {
    const idx = tabOrder.indexOf(activeTab);
    if (idx === -1) return;
    if (event.key === 'ArrowRight') { event.preventDefault(); activeTab = tabOrder[(idx + 1) % tabOrder.length]; }
    else if (event.key === 'ArrowLeft') { event.preventDefault(); activeTab = tabOrder[(idx - 1 + tabOrder.length) % tabOrder.length]; }
    else if (event.key === 'Home') { event.preventDefault(); activeTab = tabOrder[0]; }
    else if (event.key === 'End') { event.preventDefault(); activeTab = tabOrder[tabOrder.length - 1]; }
  }

  function toggleEvidence(index: number) { openEvidence = openEvidence === index ? -1 : index; }
...
>
  ...
  <div class="panel-container">
    {#if activeTab === 'findings'}
      <div id={getPanelId('findings')} class="panel" role="tabpanel" aria-labelledby={getTabId('findings')} tabindex="0">
          ...
          {#each section.evidence as evidence, index}
            ...
            <article class:open={openEvidence === index} class="evidence-card">
              ...
              {#if openEvidence === index}
                <div id={`${section.id}-evidence-${index}`} class="evidence-body" transition:slide={{ duration: 220 }}>
    ...
    {#if activeTab === 'detail'}
      <div id={getPanelId('detail')} class="panel" role="tabpanel" aria-labelledby={getTabId('detail')} tabindex="0">
          ...
          {#each section.breakdown as item, index}
            ...
            <div class="score-bar-row" style={`transition-delay:${index * 0.08}s`}>
              ...
              <div class="score-bar-track">
                <div class="score-bar-fill" style={`background:${tone.solid};width:${visible ? item.value : 0}%`}></div>
              </div>
            </div>
          {/each}
        </div>
    ...
    {#if activeTab === 'recommendations'}
      <div id={getPanelId('recommendations')} class="panel" role="tabpanel" aria-labelledby={getTabId('recommendations')} tabindex="0">
...
<style>
  ...
  .review-section.visible { opacity: 1; transform: translateY(0); }
  ...
  .evidence-main { display: flex; align-items: center; gap: 14px; }
  ...
  .evidence-toggle svg { width: 14px; height: 14px; }
  .open .evidence-toggle { transform: rotate(45deg); color: var(--color-accent); }
  ...
  .evidence-content p:last-of-type { margin-bottom: 0; }
  ...
  .source-url:hover { border-color: var(--color-accent); }
  .source-url svg { width: 11px; height: 11px; }
  ...
  @media (max-width: 720px) {
    .review-section { padding: 0 20px 64px; }
    .section-num { font-size: 30px; }
    h2 { font-size: 22px; }
    .tab-bar { overflow-x: auto; }
    .tab-bar button { padding: 7px 10px; white-space: nowrap; }
