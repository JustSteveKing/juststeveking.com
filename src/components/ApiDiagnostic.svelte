...
<div
	class="font-mono text-[13px] border-t border-b border-edge bg-surface-weak"
	role="region"
	aria-label="API diagnostic terminal"
>
	<!-- Header -->
	<div class="flex items-stretch border-b border-edge bg-surface">
		<!-- Traffic lights -->
		<div
			class="sm:hidden md:flex items-center gap-1.5 md:px-5 border-r border-edge shrink-0"
			aria-hidden="true"
		>
	...
	<div
		...
		role="tabpanel"
		class="px-section py-8 min-h-[240px]"
		aria-live="polite"
	>
		<!-- Command -->
			...
			<div class="flex flex-col gap-1.5 mb-4">
				{#each scan.findings.slice(0, visibleCount) as finding}
					<div class="flex items-baseline gap-3">
						...
						<span class="fg-weak hidden sm:block w-[13ch] shrink-0 text-right"
							>{finding.detail}</span
						>
...
<style>
	...
	.badge[data-severity='SLOW'] {
		color: #df8e1d;
	}
	.badge[data-severity='MEDIUM'] {
		color: #df8e1d;
	}
	.badge[data-severity='DESIGN'] {
		color: #1e66f5;
	}
