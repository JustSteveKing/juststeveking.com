...
export async function deployWithRollback(
	...
	if (current.status === 'failed') {
		if (lastSuccessful) {
			...
			throw new Error(`Deployment ${current.id} failed. Rolled back to ${lastSuccessful.id}`);
		}
...
// Helpers
export { createAndDeploy, deployWithRollback, provisionFullStack } from './helpers';
```
...
describe('deployWithRollback', () => {
	it('rolls back on deployment failure', async () => {
		...
		await expect(deployWithRollback(mockSevalla, 'app-1')).rejects.toThrow(/rolled back/i);

		expect(mockSevalla.applications.rollback).toHaveBeenCalledWith('app-1', 'dep-old');
	});
});
```
